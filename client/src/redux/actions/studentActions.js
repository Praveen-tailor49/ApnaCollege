...
export const getAttendance =
  ...
  (department, year) => async (dispatch) => {
    try {
      ...
      const { data } = await api.getStudentSubject(formData);
      dispatch({ type: ATTENDANCE, payload: data });
    } catch (error) {
      dispatch({ type: SET_ERRORS, payload: error.response.data });
    }
  ...
  (student) => async (dispatch) => {
    try {
      const formData = {
        student
      };
      const { data } = await api.getStudentMarks(formData);
      dispatch({ type: GET_Marks, payload: data });
    } catch (error) {
      dispatch({ type: SET_ERRORS, payload: error.response.data });
    }
  };