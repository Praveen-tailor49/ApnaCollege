...
export const getStudent = async (req, res) => {
  try {
    const { department, sem, section } = req.body;
    const errors = { noStudentError: String };
    const students = await Student.find({ department, sem, section });
  ...
  try {
    ...
    for (var i = 0; i < marks.length; i++) {
      const newMarks = await new Marks({
        ...
        exam: existingTest._id,
        internalMarks: marks[i].value,
        externalMarks: externalMarks[i].value,
        practicalMarks: practicalMarks[i].value,
        gredePoint:gredePoint[i].value,
        subject:marks[i].name,
...
export const getSubject = async (req, res) => {
  try {
    ...
    const subjects = await Subject.find({ department, sem });
